...
typedef int8_t (*icm20948_read_fptr_t)(const uint8_t addr, uint8_t *data,
                                       const uint32_t len);
typedef int8_t (*icm20948_write_fptr_t)(const uint8_t addr, const uint8_t *data,
                                        const uint32_t len);
...
/*!
 * @brief This API initializes the ICM20948 comms interface, and then does a
 * read from the device to verify working comms
 *
 ...
 * @param[in] delay: Function pointer to the developers micro-second delay
 * function
 *
 * @return Returns the status of initialization
 */
icm20948_return_code_t icm20948_init(icm20948_read_fptr_t r,
                                     icm20948_write_fptr_t w,
                                     icm20948_delay_us_fptr_t delay);

/*!
 * @brief This API applys the developers settings for configuring the ICM20948
 * components
 *
